  ...
  $ionicPlatform.ready(function() {
    ...
    if (isCordovaApp) {
      ...
      BackgroundGeolocation.configure({
        ...
        stationaryRadius: 50,
        distanceFilter: 100 ,
        notificationTitle: 'App Repartidor',
      ...
      BackgroundGeolocation.on('location', function(location) {
        console.log('[INFO] BackgroundGeolocation on location: ',location);
        BackgroundGeolocation.startTask(function(taskKey) {
          console.log('[INFO] BackgroundGeolocation taskKey: ',taskKey);
          if ($localStorage.app.sim!="") {
            ...
            var d = new Date;
            $rootScope.statusGPS = "activo";
            $localStorage.gpslat = location.latitude;
            $localStorage.gpslng = location.longitude;
            var dformat = [ (d.getMonth()+1).padLeft(), d.getDate().padLeft(), d.getFullYear()].join('/')+ ' ' + [ d.getHours().padLeft(), d.getMinutes().padLeft(), d.getSeconds().padLeft()].join(':');
            $localStorage.gpslast = dformat;
          }
      ...
      BackgroundGeolocation.on('foreground', function() {
        console.log('[INFO] BackgroundGeolocation service foreground');
      });
      BackgroundGeolocation.on('background', function() {
        console.log('[INFO] BackgroundGeolocation service background');
      });
      BackgroundGeolocation.on('stationary', function(x) {
        console.log('[INFO] BackgroundGeolocation stationary',x);
      });
      ...
      setTimeout(function() {
        ...
        cordova.plugins.backgroundMode.on('deactivate',function() {
          console.log('backgroundMode wakeup');
          $rootScope.phoneMode = "wakeup";
        });

        cordova.plugins.backgroundMode.on('activate',function () {
          console.log('backgroundMode lock');
          $rootScope.phoneMode = "lock";
          cordova.plugins.backgroundMode.disableWebViewOptimizations();  /* GPS Tracking background */
        });
